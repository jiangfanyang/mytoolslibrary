...
//	@Retention(RetentionPolicy.SOURCE)
	...
	public void updateSizes(@ProgressDrawableSize int size) {
		if (size == LARGE) {
			setSizeParameters(CIRCLE_DIAMETER_LARGE,
					CIRCLE_DIAMETER_LARGE, CENTER_RADIUS_LARGE,
					STROKE_WIDTH_LARGE, ARROW_WIDTH_LARGE,
					ARROW_HEIGHT_LARGE);
		} else {
			setSizeParameters(CIRCLE_DIAMETER, CIRCLE_DIAMETER,
	...
	public void draw(Canvas c) {
		...
		c.rotate(mRotation, bounds.exactCenterX(),
				bounds.exactCenterY());
		mRing.draw(c, bounds);
	...
	private void setupAnimators() {
		...
		final Animation finishRingAnimation = new Animation() {
			public void applyTransformation(float interpolatedTime,
				...
				float targetRotation = (float) (Math.floor(
						ring.getStartingRotation() / MAX_PROGRESS_ARC)
						+ 1f);
				final float startTrim = ring.getStartingStartTrim()
						+ (ring.getStartingEndTrim()
		...
		finishRingAnimation.setAnimationListener(
		...
		final Animation animation = new Animation() {
			...
			public void applyTransformation(float interpolatedTime,
				...
				final float minProgressArc = (float) Math
						.toRadians(ring.getStrokeWidth() / (2
								* Math.PI * ring.getCenterRadius()));
				final float startingEndTrim = ring
						.getStartingEndTrim();
				final float startingTrim = ring
						.getStartingStartTrim();
				final float startingRotation = ring
						.getStartingRotation();

				// Offset the minProgressArc to where the endTrim is located.
				final float minArc = MAX_PROGRESS_ARC
						- minProgressArc;
				...
				float groupRotation = ((720.0f / NUM_POINTS)
						* interpolatedTime)
	...
	private final Callback mCallback = new Callback() {
		...
		@Override
		public void scheduleDrawable(Drawable d, Runnable what,
				long when) {
	...
	private static class Ring {
		...
		public void draw(Canvas c, Rect bounds) {
			...
			c.drawArc(arcBounds, startAngle, sweepAngle, false,
					mPaint);
			...
			if (mAlpha < 255) {
				...
				c.drawCircle(bounds.exactCenterX(),
						bounds.exactCenterY(), bounds.width() / 2,
						mCirclePaint);
			}
		}

		private void drawTriangle(Canvas c, float startAngle,
				float sweepAngle, Rect bounds) {
			if (mShowArrow) {
				if (mArrow == null) {
					mArrow = new Path();
				...
				float x = (float) (mRingCenterRadius * Math.cos(0)
						+ bounds.exactCenterX());
				float y = (float) (mRingCenterRadius * Math.sin(0)
						+ bounds.exactCenterY());
