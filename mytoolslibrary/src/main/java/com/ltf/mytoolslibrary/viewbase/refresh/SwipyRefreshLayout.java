...
/**
 * 李堂飞 20160621 下拉刷新、加载更多、分页索引 使用方法：
 * 1.在XML中使用<chaoer.zerolif.buyer.utils.Refresh
 * .SwipyRefreshLayout>在此期间务必加载一个列表（listview或gridview
 * ）</chaoer.zerolif.buyer.utils.Refresh.SwipyRefreshLayout>
 * 2.在activity中实现implements
 * chaoer.zerolif.buyer.utils.Refresh.SwipyRefreshLayout.OnRefreshListener
 * 3.找到XML中chaoer.zerolif.buyer.utils.Refresh.SwipyRefreshLayout的id
 * 4.设置当前类刷新setOnRefreshListener(this); 5.设置刷新方式
 * setDirection(SwipyRefreshLayoutDirection
 * .BOTH);{a.top--->顶部刷新；b.BOTTOM--->底部刷新；c.BOTH--->顶部和底部都需要刷新}
 * 6.刷新完成时调用setRefreshing(false);停止刷新 为TRUE则开始刷新（不建议使用setRefreshing(true);
 * 7.刷新时建议使用：方法参数中index是分页用 例如第一次刷新时是第一页则index返回1 再次刷新时index返回1
 * 第一次上拉加载时是第一页则index返回1 再次上拉加载时index返回2 以此类推
 * 
 * @Override public void onRefresh(int index) { new Handler().postDelayed(new
 *           Runnable() {
 * @Override public void run() { ss.setRefreshing(false); } }, 5000); }
 * 
 * @Override public void onLoad(int index) { new Handler().postDelayed(new
 *           Runnable() {
 * @Override public void run() { ss.setRefreshing(false); } }, 5000); }
 *           注意：SwipeRefreshLayout和ListView的EmptyView不可共存
 */
